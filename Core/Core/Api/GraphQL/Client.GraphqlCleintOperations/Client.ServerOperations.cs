...
namespace Speckle.Core.Api
{
  public partial class Client
  {
    ...
    public async Task<bool> IsJobNumberRequiredToCreateStreams(CancellationToken cancellationToken = default)
    {
      var request = new GraphQLRequest
      {
        Query = @"query Server {
      ...
      try
      {
        var res = await ExecuteGraphQLRequest<ServerInfoWithJobNumberResponse>(request, cancellationToken).ConfigureAwait(false);
        if (res != null)
            return res.serverInfo.requireJobNumberToCreateStreams;
      } catch (Exception ex)
      {
        return false;
      }
