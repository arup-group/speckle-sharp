...
namespace ConnectorGrasshopper.Streams
{
  public class StreamCreateComponentV2 : GH_SpeckleTaskCapableComponent<StreamWrapper>
  {
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager)
    {
      ...
      var jobNumber = pManager.AddTextParameter("Job Number", "JN", "8-digit job number associated with the stream (required by Arup-hosted speckle servers, no spaces or dashes)",
        GH_ParamAccess.item);
    ...
    public override void SolveInstanceWithLogContext(IGH_DataAccess DA)
    {
      if (InPreSolve)
      {
        if (DA.Iteration == 0)
        {
          ...
          TaskList.Add(Task.Run(() => CreateStream(account, jobNumber), CancelToken));
        }
        else
        {
    ...
    public Task<StreamWrapper> CreateStream(Account account, string jobNumber = null)
    {
      if (stream != null)
      {
