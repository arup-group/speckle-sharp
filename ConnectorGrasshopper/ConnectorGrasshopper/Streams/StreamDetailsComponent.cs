    ...
    protected override void SolveInstance(IGH_DataAccess DA)
    {
      ...
      else if (streams == null)
      {
        ...
        else
        {
          ...
          var temp = Task.Run(StreamDetailsTask(ghStreamTree)).Result;
    ...
    private Func<Task<Dictionary<GH_Path, Stream>>> StreamDetailsTask(GH_Structure<GH_SpeckleStream> ghStreamTree)
    {
      return async () =>
      {
        try
        {
          ...
          var values = AccountManager.AccountFromToken == null ? await Task.WhenAll(tasks.Values) : Task.WhenAll(tasks.Values).Result;
