  ...
  public partial class ConverterGSA
  {
    ...
    private void SetupToSpeckleFns()
    {
      ToSpeckleFns = new Dictionary<Type, ToSpeckleMethodDelegate>()
      {
        ...
        { typeof(GsaPropLink), GsaPropertyLinkToSpeckle },
        //Constraints
    ...
    private ToSpeckleResult GsaAssemblyToSpeckle(GsaRecord nativeObject, GSALayer layer = GSALayer.Both)
    {
      ...
      if (gsaAssembly.Topo1.IsIndex())
      {
        ...
        AddToMeaningfulNodeIndices(speckleAssembly.end1Node.applicationId, layer);
      }
      if (gsaAssembly.Topo2.IsIndex())
      {
        speckleAssembly.end2Node = (GSANode)GetNodeFromIndex(gsaAssembly.Topo2.Value);
        AddToMeaningfulNodeIndices(speckleAssembly.end2Node.applicationId, layer);
      }
      ...
      if (gsaAssembly.IntTopo.HasValues())
      {
        ...
        AddToMeaningfulNodeIndices(intTopo.Select(n => n.applicationId), layer);
      }
    ...
    private ToSpeckleResult GsaNodeToSpeckle(GsaRecord nativeObject, GSALayer layer = GSALayer.Both)
    {
      ...
      if (Instance.GsaModel.Cache.LayerContainsNode(gsaNode.Index.Value, layer))
      {
    ...
    private GSAElement1D GsaElement1dToSpeckle(GsaEl gsaEl)
    {
      ...
      var speckleElement1d = new GSAElement1D()
      {
        ...
        //displayValue = new List<Mesh> { }, //TODO: update to handle section shape
      ...
      if (gsaEl.NodeIndices.Count >= 2)
      {
        ...
        speckleElement1d.topologyRefs = gsaEl.NodeIndices.Select(i => GetNodeFromIndex(i)).Select(n => n.applicationId).ToList();
        AddToMeaningfulNodeIndices(speckleElement1d.topologyRefs, GSALayer.Analysis);
      }
      else
      {
      ...
      if (gsaEl.PropertyIndex.IsIndex()) speckleElement1d.property = GetPropertyFromIndex(gsaEl.Type, gsaEl.PropertyIndex.Value);
    ...
    private GSAElement2D GsaElement2dToSpeckle(GsaEl gsaEl)
    {
      var speckleElement2d = new GSAElement2D()
      {
        ...
        //displayValue = new List<Mesh>() { DisplayMesh2d(gsaEl.NodeIndices) },
        orientationAngle = gsaEl.Angle ?? 0,
      ...
      if (gsaEl.NodeIndices.Count >= 3)
      {
        ...
        var nodes = gsaEl.NodeIndices.Select(i => GetNodeFromIndex(i)).ToList();
        speckleElement2d.topologyRefs = nodes.Select(n => n.applicationId).ToList();
        AddToMeaningfulNodeIndices(speckleElement2d.topologyRefs, GSALayer.Analysis);

        var points = nodes.Select(n => n.basePoint.ToList()).ToList().SelectMany(p => p).ToList();
        speckleElement2d.outline = new List<ICurve> { new Polyline(points) { closed = true } };
      }
      else
      {
    ...
    private GSAMember1D GsaMember1dToSpeckle(GsaMemb gsaMemb)
    {
      ...
      if (gsaMemb.NodeIndices.Count >= 2)
      {
        ...
        speckleMember1d.topologyRefs = gsaMemb.NodeIndices.Select(i => GetNodeFromIndex(i)).Select(n => n.applicationId).ToList();
        AddToMeaningfulNodeIndices(speckleMember1d.topologyRefs, GSALayer.Design);
      }
      else
      {
    ...
    private GSAMember2D GsaMember2dToSpeckle(GsaMemb gsaMemb)
    {
      ...
      var speckleMember2d = new GSAMember2D()
      {
        ...
        //displayValue = new List<Mesh> { DisplayMeshPolygon(gsaMemb.NodeIndices, color) },
        orientationAngle = gsaMemb.Angle ?? 0,
      ...
      if (gsaMemb.NodeIndices.Count >= 3)
      {
        ...
        outline.Add(new Polyline(coordinates) { closed = true });
        AddToMeaningfulNodeIndices(nodeAddIds, GSALayer.Design);
      }
      else
      {
      ...
      if (gsaMemb.Voids.HasValues())
      {
        ...
        var voidRefs = gsaMemb.Voids.Select(v => v.Select(i => GetNodeFromIndex(i)).Select(n => n.applicationId).ToList()).ToList();
        speckleMember2d.voidRefs = voidRefs;
        AddToMeaningfulNodeIndices(voids.SelectMany(n => n.Select(n2 => n2.applicationId)), GSALayer.Design);
      }
      ...
      if (gsaMemb.PointNodeIndices.HasValues())
      {
        var specklePoints = gsaMemb.PointNodeIndices.Select(i => (GSANode)GetNodeFromIndex(i)).Select(p => p.applicationId).ToList();
        speckleMember2d["Points"] = specklePoints;
        AddToMeaningfulNodeIndices(specklePoints, GSALayer.Design);
      }
      if (gsaMemb.Polylines.HasValues())
      {
        var speckleLines = gsaMemb.Polylines.Select(v => v.Select(i => (GSANode)GetNodeFromIndex(i)).Select(p => p.applicationId).ToList()).ToList();
        speckleMember2d["Lines"] = speckleLines;
        AddToMeaningfulNodeIndices(speckleLines.SelectMany(l => l), GSALayer.Design);
      }
      if (gsaMemb.AdditionalAreas.HasValues())
      {
        var speckleAreas = gsaMemb.AdditionalAreas.Select(v => v.Select(i => (GSANode)GetNodeFromIndex(i)).Select(p => p.applicationId).ToList()).ToList();
        speckleMember2d["Areas"] = speckleAreas;
        AddToMeaningfulNodeIndices(speckleAreas.SelectMany(a => a), GSALayer.Design);
      }
    ...
    private ToSpeckleResult GsaLoadBeamToSpeckle(GsaRecord nativeObject, GSALayer layer = GSALayer.Both)
    {
      ...
      //if (gsaLoad.MemberIndices.HasValues()) designLoad.elements = gsaLoad.MemberIndices.Select(i => GetMemberFromIndex(i)).ToList();
      if (gsaLoad.MemberIndices.HasValues()) designLoad.elementRefs = gsaLoad.MemberIndices.Select(i => GetMemberFromIndex(i)).Where(memb => memb != null).Select(m => m.applicationId).ToList();
      if (layer == GSALayer.Analysis || layer == GSALayer.Both)
      {
        //analysis layer
        ...
        //if (gsaLoad.ElementIndices.HasValues()) analysisLoad.elements = gsaLoad.ElementIndices.Select(i => GetElementFromIndex(i)).ToList();
        if (gsaLoad.ElementIndices.HasValues()) analysisLoad.elementRefs = gsaLoad.ElementIndices.Select(i => GetElementFromIndex(i)).Where(el => el != null).Select(e => e.applicationId).ToList();
      }
    ...
    private ToSpeckleResult GsaLoadNodeToSpeckle(GsaRecord nativeObject, GSALayer layer = GSALayer.Both)
    {
      ...
      if (gsaLoadNode.NodeIndices.HasValues())
      {
        ...
        speckleNodeLoad.nodeRefs = gsaLoadNode.NodeIndices.Select(i => GetNodeFromIndex(i)).Select(n => n.applicationId).ToList();
        AddToMeaningfulNodeIndices(speckleNodeLoad.nodeRefs, layer);
      }
    ...
    private ToSpeckleResult GsaLoadGravityLoadToSpeckle(GsaRecord nativeObject, GSALayer layer = GSALayer.Both)
    {
      ...
      if (gsaLoad.Nodes.HasValues())
      {
        designLoad.nodeRefs = gsaLoad.Nodes.Where(node => Instance.GsaModel.Cache.LayerContainsNode(node, layer)).Select(i => GetNodeFromIndex(i)).Select(n => n.applicationId).ToList();
        AddToMeaningfulNodeIndices(designLoad.nodeRefs, layer);
      }
      //if (gsaLoad.MemberIndices.HasValues()) designLoad.elements = gsaLoad.MemberIndices.Select(i => GetMemberFromIndex(i)).ToList();
      if (gsaLoad.MemberIndices.HasValues()) designLoad.elementRefs = gsaLoad.MemberIndices.Select(i => GetMemberFromIndex(i)).Where(memb => memb != null).Select(m => m.applicationId).ToList();
      ...
      if (layer == GSALayer.Analysis || layer == GSALayer.Both)
      {
        ...
        if (gsaLoad.Nodes.HasValues())
        {
          analysisLoad.nodeRefs = gsaLoad.Nodes.Where(node => Instance.GsaModel.Cache.LayerContainsNode(node, layer)).Select(i => GetNodeFromIndex(i)).Select(n => n.applicationId).ToList();
          AddToMeaningfulNodeIndices(analysisLoad.nodeRefs, layer);
        }
        //if (gsaLoad.ElementIndices.HasValues()) analysisLoad.elements = gsaLoad.ElementIndices.Select(i => GetElementFromIndex(i)).ToList();
        if (gsaLoad.ElementIndices.HasValues()) analysisLoad.elementRefs = gsaLoad.ElementIndices.Select(i => GetElementFromIndex(i)).Where(el => el != null).Select(e => e.applicationId).ToList();
      }
    ...
    private ToSpeckleResult GsaProperty2dToSpeckle(GsaRecord nativeObject, GSALayer layer = GSALayer.Both)
    {
      ...
      else
      {
        ...
        if (gsaProp2d.AnalysisMaterialIndex.IsIndex())
        {
          ...
          var speckleMatAnal = GetAnalysisMaterialFromIndex(gsaProp2d.AnalysisMaterialIndex.Value);
          if (speckleMatAnal != null)
          {
            speckleProperty2d.material = speckleMatAnal;
          }
          else
          {
            Report.ConversionErrors.Add(new Exception("GsaProperty2dToSpeckle: Conversion of MAT_ANAL keyword not currently fully supported"));
          }
        }
    ...
    private ToSpeckleResult GsaPropertyLinkToSpeckle(GsaRecord nativeObject, GSALayer layer = GSALayer.Both)
    {
      var gsaPropLink = (GsaPropLink)nativeObject;
      var specklePropertyLink = new GSAPropertyLink()
      {
        name = gsaPropLink.Name,
        colour = gsaPropLink.Colour.ToString(),
        nativeId = gsaPropLink.Index ?? 0,
      };
      if (gsaPropLink.Index.IsIndex()) specklePropertyLink.applicationId = Instance.GsaModel.Cache.GetApplicationId<GsaPropLink>(gsaPropLink.Index.Value);

      GSAConstraintCondition constraintCondition = new GSAConstraintCondition();
      if (gsaPropLink.Type == LinkType.Custom)
      {
        constraintCondition = GetRigidConstraint(gsaPropLink.LinkedCondition);
        specklePropertyLink.constraintCondition = constraintCondition;
      }

      var type = gsaPropLink.Type.ToSpeckle();
      specklePropertyLink.type = type;

      return new ToSpeckleResult(specklePropertyLink);
    }
    ...
    private ToSpeckleResult GsaRigidToSpeckle(GsaRecord nativeObject, GSALayer layer = GSALayer.Both)
    {
      ...
      if (gsaRigid.PrimaryNode.IsIndex())
      {
        ...
        AddToMeaningfulNodeIndices(primaryNode.applicationId, layer);
      }
      ...
      if (gsaRigid.ConstrainedNodes.HasValues())
      {
        ...
        AddToMeaningfulNodeIndices(constrainedNodes.Select(cn => cn.applicationId), layer);
      }
      ...
      var designRigid = new GSARigidConstraint()
      {
        ...
        primaryNodeRef = primaryNode.applicationId,
        constrainedNodeRefs = constrainedNodes.Select(n => n.applicationId).ToList()
      };
    ...
    private ToSpeckleResult GsaGenRestToSpeckle(GsaRecord nativeObject, GSALayer layer = GSALayer.Both)
    {
      ...
      if (gsaGenRest.NodeIndices.HasValues())
      {
        ...
        AddToMeaningfulNodeIndices(nodes.Select(n => n.applicationId), layer);
      }
      ...
      var designGenRest = new GSAGeneralisedRestraint()
      {
        ...
        //nodes = nodes,
        stages = designStages,
        nodeRefs = nodes.Select(n => n.applicationId).ToList()
      };
    ...
    private Node GetNodeFromAppId(string appId)
    {
      var index = Instance.GsaModel.Cache.LookupIndex<GsaNode>(appId);
      if (index != null) return GetNodeFromIndex((int)index);
      else return null;
    }
    ...
    private Polyline GetBasePolyline(List<string> pointAppIds)
    {
      var points = pointAppIds.Select(p => GetNodeFromAppId(p)).Select(pt => pt.basePoint).ToList();
      return GetBasePolyline(points);
    }
    ...
    private Line GetBaseLine(List<string> pointAppIds)
    {
      var points = pointAppIds.Select(p => GetNodeFromAppId(p)).Select(pt => pt.basePoint).ToList();
      return GetBaseLine(points);
    }
    ...
    private Property GetPropertyFromIndex(ElementType elementType, int index)
    {
      if (elementType == ElementType.Link)
      {
        return (Instance.GsaModel.Cache.GetSpeckleObjects<GsaPropLink, PropertyLink>(index, out var speckleObjects) && speckleObjects != null && speckleObjects.Count > 0)
        ? speckleObjects.First() : null;
      }
      else
      {
