...
namespace ConnectorGSA
{
  public static class Commands
  {
    ...
    public static bool ConvertToNative(List<Base> objects, ISpeckleConverter converter, ProgressViewModel progress) //Includes writing to Cache
    {
      try
      {
      ...
      {
        progress.Report.LogConversionError(ex);
      }
    ...
    private static async Task<bool> Send(TabCoordinator coordinator, StreamState ss, IProgress<MessageEventArgs> loggingProgress, IProgress<string> statusProgress, IProgress<double> percentageProgress)
    {
      ...
      //var sent = await Commands.SendCommit(commitObj, ss, progress, ((GsaModel)Instance.GsaModel).LastCommitId, serverTransport);
      ...
      IProgress<MessageEventArgs> loggingProgress, IProgress<string> statusProgress, IProgress<double> percentageProgress)
    {
      ...
      {
        var stream = await NewStream(client, "GSA data", "GSA data");
        streamState = new StreamState(account, stream);
    ...
    public static string UnitEnumToString(GsaUnit unit)
    {
      switch (unit)
      {
    ...
    private static System.Version getRunningVersion()
    {
      try
      {
        return Assembly.GetExecutingAssembly().GetName().Version;  //ApplicationDeployment.CurrentDeployment.CurrentVersion;
      }
      catch (Exception)
      {
